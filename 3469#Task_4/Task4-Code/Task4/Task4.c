...
void Wall_run(void)
{
	forward_wls(0);			//1st Node
	right_turn_wls();
	forward_wls(2);			//2nd Node
	right_turn_wls();
	forward_wls(0);			//3rd Node
	forward();
	_delay_ms(250);
	left_turn_wls();
	back();
	...
	forward_wls(0);			//4th Node
	forward_wls(0);			//5th Node
	forward();
	_delay_ms(250);
	left_turn_wls();
	back();
	...
	forward_wls(1);			//Wall Following, 14th Node
	static_reorientation();
	m_place_lr(); // <----------- 1st Place Master
	forward();
	_delay_ms(300);
	left_turn_wls();
	forward_wls(0);			//13th Node
	static_reorientation();
	forward_wls(0);			//12th Node
	right_turn_wls();
	back();
	_delay_ms(300);
	stop();
